...
void optimized_6(int np, int num_runs) {
    ...
    for (int a = 0; a < n_p; a+=4){
        ...
        _mm256_store_pd(&vbs_x[a], _mm256_mul_pd(_mm256_loadu_pd(&v_x[a]), D_t));
        _mm256_store_pd(&vbs_y[a], _mm256_mul_pd(_mm256_loadu_pd(&v_y[a]), D_t));
    ...
    for (int i = 0; i < n_p; i += 4) {
        ...
        __m256d _e_x = _mm256_load_pd(&e_x[i]);
        __m256d _e_y = _mm256_load_pd(&e_y[i]);

        __m256d _v_x = _mm256_load_pd(&v_x[i]);
        __m256d _v_y = _mm256_load_pd(&v_y[i]);
        ...
        __m256d r_i_x1 = _mm256_broadcast_sd(&r_x[i+1]);
        __m256d r_i_y1 = _mm256_broadcast_sd(&r_y[i+1]);

        __m256d r_i_x2 = _mm256_broadcast_sd(&r_x[i+2]);
        __m256d r_i_y2 = _mm256_broadcast_sd(&r_y[i+2]);

        __m256d r_i_x3 = _mm256_broadcast_sd(&r_x[i + 3]);
        __m256d r_i_y3 = _mm256_broadcast_sd(&r_y[i + 3]);

        __m256d e_i_x = _mm256_broadcast_sd(&e_x[i]);
        __m256d e_i_y = _mm256_broadcast_sd(&e_y[i]);

        __m256d e_i_x1 = _mm256_broadcast_sd(&e_x[i+1]);
        __m256d e_i_y1 = _mm256_broadcast_sd(&e_y[i+1]);

        __m256d e_i_x2 = _mm256_broadcast_sd(&e_x[i+2]);
        __m256d e_i_y2 = _mm256_broadcast_sd(&e_y[i+2]);

        __m256d e_i_x3 = _mm256_broadcast_sd(&e_x[i + 3]);
        __m256d e_i_y3 = _mm256_broadcast_sd(&e_y[i + 3]);
        ...
        for (int j = 0; j < n_p; j += 4) {
            ...
            __m256d dot_prod = _mm256_fmadd_pd(e_i_x, f_ab_x, _mm256_fmadd_pd(e_i_y1, f_ab_y, all_zeros)); // e[i].dot(-f_ab)
            __m256d f_ab_norm = _mm256_sqrt_pd(_mm256_fmadd_pd(f_ab_x, f_ab_x, _mm256_mul_pd(f_ab_y, f_ab_y)));
            __m256d comp = _mm256_mul_pd(all_cos_phi, f_ab_norm);
        ...
        for (int j = 0; j < n_p; j += 4) {
            ...
            __m256d dot_prod = _mm256_fmadd_pd(e_i_x, f_ab_x, _mm256_fmadd_pd(e_i_y2, f_ab_y, all_zeros)); // e[i].dot(-f_ab)
            __m256d f_ab_norm = _mm256_sqrt_pd(_mm256_fmadd_pd(f_ab_x, f_ab_x, _mm256_mul_pd(f_ab_y, f_ab_y)));
            __m256d comp = _mm256_mul_pd(all_cos_phi, f_ab_norm);
        ...
        for (int j = 0; j < n_p; j += 4) {
            ...
            __m256d rab_x = _mm256_sub_pd(r_i_x3, r_j_x);
            __m256d rab_y = _mm256_sub_pd(r_i_y3, r_j_y);
            ...
            __m256d dot_prod = _mm256_fmadd_pd(e_i_x, f_ab_x, _mm256_fmadd_pd(e_i_y3, f_ab_y, all_zeros)); // e[i].dot(-f_ab)
            __m256d f_ab_norm = _mm256_sqrt_pd(_mm256_fmadd_pd(f_ab_x, f_ab_x, _mm256_mul_pd(f_ab_y, f_ab_y)));
            __m256d comp = _mm256_mul_pd(all_cos_phi, f_ab_norm);
        ...
        __m256d _raB0_y = _mm256_set_pd(r_y[i], r_y[i + 1], r_y[i + 2], r_y[i + 3]);
        ...
        // // AVX
        double* _r_next_x = &r_x[i];
        double* _r_next_y = &r_y[i];
        double* _vMax = &vMax[i];
        __m256d __r_next_x = _mm256_loadu_pd(_r_next_x);
        __m256d __r_next_y = _mm256_loadu_pd(_r_next_y);
        __m256d __v_x = _mm256_loadu_pd(&v_x[i]);
        __m256d __v_y = _mm256_loadu_pd(&v_y[i]);
        ...
        _mm256_store_pd(&r_x[i], __r_next_x);
        _mm256_store_pd(&r_y[i], __r_next_y);
...
void register_functions() {
    ...
    add_function(&optimized_6, "optimized_6");
    // add_function(&optimized_5, "optimized_5");
